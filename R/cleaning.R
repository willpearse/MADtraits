...
#' @note See \code{\link{convert.MADtraits.units}} and
#'     \code{\link{lookup.MADtraits.names}} for functions to match
...
#' @author Will Pearse
#' @seealso convert.MADtraits.units lookup.MADtraits.names
#' @examples
...
#' @seealso clean.MADtraits convert.MADtraits.units
...
#' demo <- lookup.MADtraits.names(demo, use.cache=TRUE)
#' #...in this case with only six species it was fast
lookup.MADtraits.names <- function(x, thresh=0.8, use.cache=TRUE){
    # Argument handling
    if(!inherits(x, "MADtraits"))
        stop("'", deparse(substitute(x)), "' must be of type 'MADtraits'")
...
#' @return \code{\link{MADtraits}} object
#' @author Will Pearse and Kathryn M. Welglarz
#' @seealso clean.MADtraits lookup.MADtraits.names
#' @examples
#' # Grab some example data (you should work with the output from the MADtraits function)
#' demo <- .pearse.2014()
#' # Convert all the units to the most common type
...
convert.MADtraits.units <- function(x, choices){
    # Argument handling
    if(!inherits(x, "MADtraits"))
        stop("'", deparse(substitute(x)), "' must be of type 'MADtraits'")
