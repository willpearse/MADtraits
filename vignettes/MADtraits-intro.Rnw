...
functions \texttt{convert.MADtraits.units} and
\texttt{lookup.MADtraits.names} help with `cleaning up' units and
taxonomic names within \MADtraits. Give these a try---in particular,
you will likely not want to work with data whose units you haven't
made the same throughout as otherwise you'll be multiplying apples by
oranges!
