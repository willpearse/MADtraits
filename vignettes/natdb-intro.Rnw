...
\texttt{devtools::install_github("willpearse/fulltext")} and then
\texttt{devtools::install_github("willpearse/natdb")}. Once the \natdb
manuscript is accepted somewhere, a new version of \texttt{fulltext}
...
<<>>=
library(natdb)                                # 1
data <- natdb(cache="~/Code/natdb/cache")     # 2
...
<<>>=
library(fulltext)
tree <- read.nexus(ft_get_si("10.1111/j.1461-0248.2009.01307.x", 1))[[1]]
@ 
...
data (again using \texttt{fulltext}):
...
\texttt{ft_get_si} from \texttt{fulltext}---all it requires is the DOI
...
that data. I maintain that function in \texttt{fulltext}, so if it's
